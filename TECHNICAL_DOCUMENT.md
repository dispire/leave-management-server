...
## 🤖 신규 프로젝트: 윈도우 파워오토메이트(Power Automate Desktop) 반복작업 자동화 봇 (`PowerAutomateBot`) (2026-09-22)

### 📌 프로젝트 개요 및 목적
- **프로젝트명**: 윈도우 Power Automate 기반 반복작업 자동화 봇 탐색 (`PowerAutomateBot`)
- **목적**: 윈도우 10/11 환경에서 기본 탑재/제공되는 Microsoft Power Automate Desktop (PAD) RPA 도구를 활용하여 매일 반복되는 업무(웹 스크래핑, 엑셀 정리, 이메일 분류, 데스크톱 GUI 조작 등)를 자동화하는 방안 구축.

### 🤖 탐색된 주요 활용 방안 및 아키텍처
1. **웹 자동화 (Web Automation)**: Chrome/Edge 확장 프로그램과 결합하여 로그인, 서식 입력, 데이터 스크래핑 자동화.
2. **엑셀 & 문서 자동화 (Excel Automation)**: 백그라운드 엑셀 데이터를 Read/Write 및 리포트 양식 생성.
3. **데스크톱 앱 조작 (Desktop RPA)**: API가 없는 레거시 윈도우 응용 프로그램 마우스/키보드 자동 조작.
4. **하이브리드 봇 (Python + Power Automate)**:
   - 파워오토메이트의 GUI 조작 능력과 Python의 데이터 처리(Pandas/OpenPyXL) 및 Telegram 알림 기능 연동.
   - 윈도우 작업 스케줄러(Task Scheduler)를 통한 무인 자동 실행(Unattended Automation) 환경 탐색.

### 📝 향후 세부 개발 및 가이드 계획
- 사용자 맞춤형 반복 작업 시나리오 수립 (웹 입력 / 엑셀 정리 / 이메일 자동화 등)
- Python 연동 헬퍼 스크립트 (`bot_scheduler.py`, `notifier.py`) 제공 및 매뉴얼 구축.
