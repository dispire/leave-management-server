...
- [x] 역 선택 모달 레이어 미닫힘으로 인한 날짜 클릭 간섭 및 시간 셀렉터 오매칭 버그 완벽 수정 (2026-09-21):
  - 역 선택 후 `.ReactModalPortal` 모달이 닫히지 않아 `a.btn_d-day` 클릭 시 pointer-events 간섭이 발생하는 문제 해결(`dismissPopup` 즉시 호출).
  - 시간 선택 시 한 자릿수시간(`8시`)과 코레일 2자릿수시간(`08시`)을 모두 파싱하도록 정수형 숫자 매칭(`parseInt(txt) === targetHourInt`)으로 정밀도 대폭 향상.
- [x] 코레일 회원 로그인 제출 버튼 셀렉터 및 세션 유지 처리 수정 (2026-09-21):
  - 기존 로그인 버튼 locator `a.btnGoLogin, button.btn_bn-depblue`가 GNB 상단 헤더 메뉴 링크(`a.btnGoLogin`)를 우선 선택하여 폼 제출 대신 로그인 페이지가 새로고침되던 현상 완벽 수정.
  - 실제 폼 제출 버튼(`button.btn_bn-depblue`, `button:has-text("로그인")`) 타겟팅 및 비밀번호 엔터 제출(`press('Enter')`) 적용.
  - 로그인 성공 후 `로그아웃` 및 `회원명(이광희 님)` 세션 검증 로직 구현 및 검색 페이지 이동 시 회원 세션 정상 유지 확인.
